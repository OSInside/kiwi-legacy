...
\hspace*{1.9cm}name="iname" \textit{inherit="path">}\\
\hspace*{1cm}...\\
</image>
\end{Command}
...
by KIWI. The optional attribute inherit allows to inherit the
packages information from another image description. Inside the
\textbf{image} section the following information applies.
...
\begin{Command}{12cm}
<users group="users">\\
\hspace*{1cm}<user pwd=".." home="dir" name="user"/>\\
\hspace*{1cm}...\\
</users>
...
\end{Command}
...
\item \textbf{opensuse://<Project-Name>}\\
      ...
      For example: path=''opensuse://openSUSE:10.3/standard''
\item \textbf{local path}\\
