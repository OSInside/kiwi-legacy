...
sub addArchives {
	...
	my %verifyData = (
		caller       => 'addArchives',
		expectedType => 'KIWIXMLArchiveData',
		itemName     => 'archives',
		itemsToAdd   => $archives,
		profNames    => $profNames,
		type         => $imageType
	...
	for my $pckgNd (@pckgsNodes) {
		...
		for my $prof (@profsToProcess) {
			for my $archiveNd (@archiveNodes) {
				...
				my $archiveObj = KIWIXMLArchiveData -> new($kiwi, \%archData);
				my $accessID;
				if ($bootIncl && $bootIncl eq 'true') {
					$accessID = 'bootArchives';
				} else {
	...
	for my $pckgNd (@pckgsNodes) {
		...
		for my $prof (@profsToProcess) {
			for my $ignoreNd (@ignoreNodes) {
				...
				my $ignoreObj = KIWIXMLIgnorePackageData
					-> new($kiwi, \%ignoreData);
				my %storeData = (
					accessID => 'ignorePkgs',
					arch     => $arch,
	...
	for my $pckgNd (@pckgsNodes) {
		...
		for my $prof (@profsToProcess) {
			for my $prodNd (@productNodes) {
				...
				my $prodObj = KIWIXMLProductData -> new($kiwi, \%productData);
				my %storeData = (
					accessID => 'products',
					arch     => $arch,
					dataObj  => $prodObj,
